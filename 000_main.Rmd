...
```{r join_everything, include=FALSE}
...
# Variable weigthing
eu_vars <- eu_vars %>% 
    mutate_at(c("trust_natgov", "trust_eu", "eqi_2013", "soc_idx",
                "migration_2013", "sgi_aggregated", "agr_share",
                "eu_id_positive", "eu_id_neutral", "eu_id_negative", "eu_id_dk"),
              funs(. * weight))

# remove regions with missing values for the dependent variable (eu_id);
...
Variables that were weighted with the population of 2014 per NUTS2 region:
...
*Please note that due to the weigthing, the values of the weighted variables do not correspond any more to those that are shown in this report. However, the weights are easily removed by dividing the variable with the corresponding `weight`.*
...
All regions included, even those that contain missing values for the independent variables. Regions are excluded that are missing values for the dependent variable `eu_id_typ_code`.
