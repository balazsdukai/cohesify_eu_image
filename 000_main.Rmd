...
eu_vars_ri <- eu_vars %>%
    filter(!is.na(eu_vars$eu_id_type_code)) %>% 
    select(-ends_with(".incl"))
# these regions are removed
eu_vars_ri_n <- eu_vars %>% 
    filter(is.na(eu_vars$eu_id_type_code)) %>% 
    select(id2.2013)
write_csv(eu_vars_ri, path = "~/Development/cohesify/results/eu_all.csv")
...
write_csv(eu_vars_no_na, path = "~/Development/cohesify/results/eu_no_missing.csv")
...
write_csv(eu_vars_include_n, path = "~/Development/cohesify/results/eu_include.csv")
