...
```{sql connection=db, output.var="shp_2010_0"}
select distinct nuts_id
from nuts_2010_poly
where stat_levl_ = 0
```

The countries that are not included in the Excel sheet but included in the shapefile.
```{r}
setdiff(n2010_0$Code.2010, shp_2010_0$nuts_id)
setdiff(shp_2010_0$nuts_id, n2010_0$Code.2010)
```
