...
```{r pop_weigths, include=TRUE}
nuts_2013_weigths <- pop_2014 %>% 
    filter(nutslvl == 2) %>% 
    inner_join(nuts_2013_levels, by = c("nuts_2013" = "id2")) %>% 
    rename(id2 = nuts_2013) %>% 
    select(id0, id1, id2, population)

for (n2 in nuts_2013_weigths$id2) {
    id0 <- unique(nuts_2013_weigths[nuts_2013_weigths$id2 == n2, "id0"])
    tp <- pop_2014[pop_2014$nuts_2013 == id0, "population"]
    rp <- nuts_2013_weigths[nuts_2013_weigths$id2 == n2, "population"]
    w <- rp / tp
    nuts_2013_weigths[nuts_2013_weigths$id2 == n2, "w"] <- w
}
```
