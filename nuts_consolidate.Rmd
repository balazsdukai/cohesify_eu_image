...
```{bash shp2pgsql_2013, echo=FALSE, include=FALSE}
...
tr_2006_2010 <- dbReadTable(db, "nuts_translate_2006_2010")
```
...
```{r}
qog_translate <- nuts_2010_levels %>%
    select(id2, id3) %>% 
    full_join(tr_2006_2010, by = c("id3" = "nuts2010")) %>% 
    full_join(nuts_2006_levels, by = c("nuts2006" = "id3")) %>% 
    inner_join(qog_raw, by = c("id2.y" = "NUTS")) %>% 
    select(id2.x, EQI.2013) %>% 
    distinct()
```

Compare the translated and the original to check if everything is correct.

```{r check_qog}
no_match <- full_join(qog_translate, qog_raw_nuts2, by = c("id2.x" = "NUTS")) %>% 
    filter(is.na(EQI.2013)) %>% 
    arrange(id2.x)
```
...
What's with *HR04*? Its not in 2006 but it is in 2010 and it is not recorded in the NUTS conundrum.
...
The coverage is heterogenous, some countries have NUTS2 level poverty values, while others are aggregae to NUTS0 level.
...
```{r}
acc2soc_translate <- nuts_2010_levels %>%
    select(id2, id3) %>% 
    full_join(tr_2006_2010, by = c("id3" = "nuts2010")) %>% 
    full_join(nuts_2006_levels, by = c("nuts2006" = "id3")) %>% 
    inner_join(acc2soc_raw, by = c("id2.y" = "NUTS2")) %>% 
    select(id2.x, SGI.aggreg.) %>% 
    distinct()
```
...
```{r check_acc2soc}
no_match <- full_join(acc2soc_translate, acc2soc_raw, by = c("id2.x" = "NUTS2")) %>%
    select(id2.x, SGI.aggreg..x) %>% 
    filter(is.na(SGI.aggreg..x)) %>% 
    arrange(id2.x)
```
