...
trust_level <- function(x, trust) {
    ...
    for (ctry in id2) {
        ...
        x[x["id2"] == ctry, trust_value] <- tend_to_trust / total
    }
...
trust_pct <- trust_level(data.frame(nt), "trust_natgov") %>% 
    ...
    distinct() %>% 
    arrange(id2) %>% 
    mutate(id0 = substr(id2, 0, 2))

```

```{r trust_plot, echo=FALSE}
ggplot(trust_pct, aes(trust_natgov_value, trust_eu_value, color = id0)) +
    geom_point()
```
...
1. How to aggregate the opinions?

2. Probably I found the weights, but where is their description?
...
1. Do we need */SF eligibility/Database of the cumulative allocations to selected projects and expenditure at NUTS2 2007-2013.xlsx* *sheet:DB_NUTS2* *col: Objective*?

2. What are the programe codes? Which are relevant?

3. How to aggregate classes?

4. Again, this won't need population weighting.
