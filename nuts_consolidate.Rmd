...
Population data from 2014 is used to generate region weights, because until 2014 *FRA, SI, UKI* were not recording population values.
...
Join the NUTS3 2006 variable to the NUTS2-3 2010 regions using the 2006 <-> 2010 translation schema, then translate to NUTS 2013.
...
knitr::kable(head(typ_border_clean), type = "html", title = "A sample of the translated \' Territorial boundaries á border regions \' variable")
```

```{r clean_up2, include=TRUE}
...
Data quality check that summarizes how many variables are missing per region level.
...
val_lvl1 <- trust_raw %>%
    ...
    rename(trust_natgov = qa8a_7, trust_eu = qa8a_9, id2 = id2.2013) %>% 
    select(id2, trust_natgov, trust_eu)
...
```{r, results='asis', echo=FALSE}
knitr::kable(head(trust_clean), caption = "Trust in national government, NUTS 2013 level 2")
```

```{r clean_up3, include=TRUE, message=FALSE, warning=FALSE}
# clean up
rm(nt, trust_be, trust_to_lvl2, trust_translate, val_lvl0, val_lvl1, eu, coverage_id, nuts_2010_levels_iso, trust_pct, trust_raw)
gc()
```
...
Translate NUTS 2006 to 2010 to 2013. I know it could be done straight to 2013 but the original idea was to use 2010 version.
...
```{r qog_translate_2013, include=TRUE}
...
```{r clean_up4, include=TRUE, message=FALSE, warning=FALSE}
rm(qog_translate, no_match, qog_raw, qog_raw_nuts2)
gc()
```
...
```{r clean_up5, include=TRUE, message=FALSE, warning=FALSE}
rm(struct_2014_prep, struct_2007_prep, struct_raw_2007, struct_raw_2014)
gc()
```
...
*Europe 2020 Index based on the distances to 4 of the 8 Europe 2020 Strategy*
...
```{r clean_up6, include=TRUE, message=FALSE, warning=FALSE}
...
Translate NUTS 2006 to 2010 to 2013.
...
```{r clean_up7, include=TRUE, message=FALSE, warning=FALSE}
...
```{r clean_up8, include=TRUE, message=FALSE, warning=FALSE}
rm(missing)
```
